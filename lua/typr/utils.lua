...
M.words_to_lines = function()
    ...
    for i = 1, state.linecount do
        ...
        while lineLength < maxw do
            ...
            if lineLength + vim.fn.strchars(word) + 1 > maxw then
                break
            end
            table.insert(lineWords, word)
            lineLength = lineLength + vim.fn.strchars(word) + 1 -- +1 for the space
        end
...
M.phrases_to_lines = function()
    ...
    while state.linecount >= #lines do
        for _, v in ipairs(vim.split(phrase, " ")) do
            local arrlen = #lines
            local lastline_len = vim.fn.strchars(lines[arrlen])

            if lastline_len + vim.fn.strchars(v) + 2 > maxw then
...
M.gen_lines_diff = function(line, userline)
    local result = {}
    local userline_chars = utf8_chars(userline)
    local line_chars = utf8_chars(line)

    for i = 1, #userline_chars do
        local char = userline_chars[i]
        local expected = line_chars[i]
    ...
    local rest = table.concat(vim.list_slice(line_chars, #userline_chars + 1))
    table.insert(result, { rest, "commentfg" })
...
M.count_correct_words = function()
    ...
    for _, v in ipairs(userlines) do
        ...
        if v[2] == "Added" then count = count + wordslen
            curchar_count = curchar_count + vim.fn.strchars(v[1])
        elseif v[2] == "Removed" and v[1] ~= " " and v[1] ~= "" then
            unmatched_count = unmatched_count + wordslen
        end

        total_char_count = total_char_count + vim.fn.strchars(v[1])
    end
...
M.get_accuracy = function()
    ...
    local mystrlen = vim.fn.strchars(mystr)
    local total_char_count = vim.fn.strchars(table.concat(state.default_lines))
...
M.char_accuracy = function()
    local userlines = vim.tbl_map(function(line)
        ...
        return utf8_chars(userstrs)
    end, state.ui_lines)

    local default_lines = vim.tbl_map(function(line)
        return utf8_chars(line)
